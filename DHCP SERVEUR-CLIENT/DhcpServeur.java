...
public class DhcpServeur {
    String gateway;
    Vector<String> IPdispo;
    int indDispo;

    public String getAppropriateIP() throws Exception {
        indDispo = indDispo + 1;
        if (IPdispo.get(indDispo - 1) != null) {
            return IPdispo.get(indDispo - 1);
        } else {
            throw new Exception("plus d'IP dispo");
        }
    ...
    public DHCPPacket createPacket(int xid, InetAddress mIP, InetAddress mac, byte messageType) throws Exception {
        ...
        if (true) {
            dhcpPacket.setGiaddr(InetAddress.getByName(gateway));
        ...
        if (messageType == (byte) 5) {
            // dhcpPacket.setCiaddr(InetAddress.getAllByName(getAppropriateIP()));
        }
